...
Plik został utworzony. Zauważyłem, że Twoje obecne pliki znajdują się w folderze `documentation/`, podczas gdy instrukcja od profesora wymaga folderu `/docs`. Na tym etapie to nie przeszkadza, ale przed samym oddaniem projektu warto będzie zmienić nazwę folderu z `documentation` na `docs`, aby trzymać się sztywno wytycznych z punktu 2 instrukcji.

Czy chcesz, abym teraz pomógł Ci dostosować strukturę ról w `/docs/roles` (Product Owner, UX/UI itd.), aby spełniały wymagania z punktu 7?

<!--
[PROMPT_SUGGESTION]Przygotuj szablony dla ról w documentation/roles zgodnie z punktem 7 instrukcji.[/PROMPT_SUGGESTION]
[PROMPT_SUGGESTION]Zmień nazwę folderu documentation na docs i zaktualizuj odniesienia w plikach.[/PROMPT_SUGGESTION]
